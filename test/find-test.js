...
tape("quadtree.find(x, y, radius) returns the closest point within the search radius to the given [x, y]", function(test) {
  ...
  test.deepEqual(q.find(20, 20, 20 * Math.SQRT2 - 1e-6), []);
  test.deepEqual(q.find(0, 20, 20 + 1e-6), [[0, 0]]);
  test.deepEqual(q.find(0, 20, 20 - 1e-6), []);
  test.deepEqual(q.find(20, 0, 20 + 1e-6), [[0, 0]]);
  test.deepEqual(q.find(20, 0, 20 - 1e-6), []);
  test.end();
});
...
tape("quadtree.find(x, y) returns all coincident points", function(test) {
  var q = d3_quadtree.quadtree([[0, 0, "one"], [1, 0, "two"], [1, 0, "three"], [1, 0, "four"]]);
  test.deepEqual(q.find(1, 0), [[1, 0, "four"], [1, 0, "three"], [1, 0, "two"]]);
  test.end();
});
